...
def search_documents(question, number=3):
    if brain.count() == 0:
        return []
    try:
        results = brain.query(query_texts=[question], n_results=min(number, brain.count()))
...
def remember_exchange(question, answer):
    exchange_id = hashlib.md5(f"{question}{answer}".encode("utf-8")).hexdigest()
    memory.add(ids=[exchange_id], documents=[f"User: {question}\nNexa: {answer}"], metadatas=[{"type": "conversation"}])

def recall_memory(question, number=2):
    if memory.count() == 0:
        return []
    try:
        results = memory.query(query_texts=[question], n_results=min(number, memory.count()))
        documents = results.get("documents")
        if not documents:
            return []
        return documents[0]
    ...
    try:
        ...
        def search_web(question):
            if tavily is None:
                return ""
            try:
                results = tavily.search(query=question, max_results=5, search_depth="advanced")
                web_context = ""
...
.nexa-header {
    padding: 25px 30px;
    border-radius: 20px;
    background: linear-gradient(135deg, #0E171F 0%, #0A1118 50%, #07110D 100%);
...
.card {
    ...
    border-radius: 18px;
    padding: 20px;
    box-shadow: 0 8px 30px rgba(0,230,118,.06);
}
.card-title {
    color: #8B98A7;
    font-size: 13px;
...
if prompt:
    ...
    with st.chat_message("user"):
        st.markdown(prompt)
    document_results = search_documents(prompt, remember_documents)
    memory_results = recall_memory(prompt, recall)
    web_context = ""
    if web_search:
    ...
    if document_results:
        ...
        document_context = "No relevant document information was found."
    if memory_results:
        memory_context = "\n\n".join(memory_results)
    else:
        memory_context = "No relevant previous memories were found."
    ...
    with st.chat_message("assistant"):
        with st.spinner("Nexa is thinking... ✦"):
            try:
                ...
                print("Total message characters:", sum(len(str(m["content"])) for m in messages))
                response = client.chat.completions.create(model="llama-3.3-70b-versatile", messages=messages, temperature=creativity)
